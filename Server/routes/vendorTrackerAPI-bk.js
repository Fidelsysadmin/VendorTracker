...
router.get('/getFile', function(req, res,next){
    ...
    var fileName = '0000-ENG-MLT-220801-2_002.pdf';
    res.sendFile(fileName, options, function (err) {
        if (err) {
            ...
            next();
            // res.attachment(path.resolve('0000-ENG-MLT-220801-2_002.pdf'))
            // res.send()
        }
    });
            // res.attachment(path.resolve('0000-ENG-MLT-220801-2_002.pdf'))
...
router.post('/uploadRequestFile', (req, res) => {
    ...
    file.mv(path, (err) => {
        ...
        myConn.query("INSERT INTO request(dor_time, request_from, e_code, co_ordinator,source_language, target_language, service_type, particulars,  status,created_by,date_of_closure,filename,mimetype,path) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",[dateOfRequest,requestFrom,eCode,coOrdinator,strSourceLanguage,strTargetLanguage,strServiceType,particulars,status,createdBy,closureDate,filename,mimetype,path],(err,result) => {
            if(!err)
             {
...
// update request
router.post("/requestFileUpdate",(req,res)=>{
     let dateOfRequest = req.body.dor_time;
    let requestFrom = req.body.request_from;
   ...
   console.log(req.body)
    if(req.files.length != 0)
    {
        ...
        if(userRole === 'VM' || userRole === 'VC')
        {
            ...
            if(doc_vmpm == '' || doc_vmpm == 'Invalid date' || doc_vmpm == null || doc_vmpm == 'null' )
            {
            ...
            } else 
            {
                    ...
                    console.log('vmpc'+req.body.doc_vmpm)
    ...
    } else
    {
            ...
            if(userRole === 'VM' || userRole === 'VC')
            {
                ...
                {
                myConn.query("UPDATE request set dor_time = ? , request_from = ? , e_code = ? , co_ordinator = ? , source_language = ? , target_language = ? , service_type = ? , particulars = ? , status = ? ,date_of_closure = ? ,doc_vmpm = ? where id = ? ",[dateOfRequest,requestFrom,eCode,coOrdinator,strSourceLanguage,strTargetLanguage,strServiceType,particulars,status,doc,doc_vmpm,id],(err,result) =>{
                        if(!err)
                        {
                            res.json({
                                status:1,
                ...
                } else 
                {
                    ...
                    myConn.query("UPDATE request set dor_time = ? , request_from = ? , e_code = ? , co_ordinator = ? , source_language = ? , target_language = ? , service_type = ? , particulars = ? , status = ? ,date_of_closure = ?  where id = ? ",[dateOfRequest,requestFrom,eCode,coOrdinator,sourceLanguage,strTargetLanguage,serviceType,particulars,status,doc,id],(err,result) =>{
                        if(!err)
                        {
                            res.json({
                                status:1,
